...
Biến `bear`, `favoriteFoods` và `sum` thực chất chỉ lưu trữ địa chỉ tham chiếu tới giá trị trong bộ nhớ _Heap_, chứ không trực tiếp lưu trữ chúng.
...
Lúc này, cả `bear` và `bearCopy` đều tham chiếu đến cùng một object trong bộ nhớ _Heap_, nên sửa một biến sẽ thấy sự thay đổi thông qua biến còn lại.
...
==Nguyên là AI trong cuộc đời này==
