...
<Image
  ...
  caption='Hinh 1: Primitive types trong JavaScript'
/>
...
<Image
  ...
  alt='Image test unsplash'
  caption='Hinh 2: Lưu trữ giá trị nguyên thủy trong bộ nhớ'
/>
...
<Image
  ...
  alt='Image test unsplash'
  caption='Hinh 3: Sửa giá trị của biến nguyên thủy'
/>
...
<Image
  ...
  alt='Image test unsplash'
  caption='Hinh 4: Gán giá trị của biến nguyên thủy cho biến khác'
/>
...
<Image
  ...
  alt='Image test unsplash'
  caption='Hinh 5: Sửa giá trị của biến nguyên thủy'
/>
...
<Callout status="info">
  Các bạn tránh hiểu nhầm việc *giá trị nguyên thủy không thể thay đổi* với việc
  *biến không thể thay đổi*. Khi bạn gán một giá trị mới cho một biến, biến đó sẽ
  trỏ đến một vị trí bộ nhớ mới chứa giá trị mới, không phải thay đổi giá trị
  cũ.

</Callout>

Ở thí dụ dưới đây, giá trị `Hello` là bất biến. Còn việc tạo ra một chuỗi mới `Hi` thì đây là một giá trị nguyên thủy mới và được gán vào biến `greeting` thôi chứ không phải là thay đổi giá trị `Hello` ban đầu.
...
<Details title="What is Deep copy?">
`next-intl` uses [`cache()`](https://react.dev/reference/react/cache) to create a mutable store that holds the current locale. By calling `unstable_setRequestLocale`, the current locale will be written to the store, making it available to all APIs that require the locale.
